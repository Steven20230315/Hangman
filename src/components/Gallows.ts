...
const lengthBaseOnBase = ({ $base = 10 }: Base) => (ratio: number) => `${$base * ratio * 25}px`;

const marginLeftBaseOnBase = ({ $base = 10 }: Base) => `${$base * 12}px`;

const headBorderBaseOnBase = ({ $base = 10, $backgroundColor='black' }: GallowsProps ) => `${$base *0.5 }px solid ${$backgroundColor}`;
...
/* TODO: Add frowny face and smiley face animation */
	...
	.base, .top-bar, .rope, .pillar, .body, .right-arm, .left-arm, .right-leg, .left-leg,.frown, .left-eye, .right-eye {
